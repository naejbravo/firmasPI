...
foreach ($data as $dato) {
	/*$sql = "INSERT INTO horarios_firmados (fecha, id_horario, id_firma, fecha_firma) VALUES (curdate(), " . $dato['id'] . ", (SELECT id_firma FROM firmas where firma = '" . $data2 . "'), current_timestamp)";*/
	$sql = "UPDATE horarios_firmados SET id_firma = (SELECT id_firma FROM firmas WHERE firma = '" . $data2 . "'), fecha_firma = current_timestamp WHERE fecha = curdate() AND id_horario = " . $dato['id'] . "";
